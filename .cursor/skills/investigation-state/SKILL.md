...
- After **three** consecutive CONTINUE days with no new primary excerpt, or two days with the same `next_action`, apply the same-object test. If it passes, WRITE. If it fails, park or kill. Write why in `gaps.md`. Repeating an unchanged API query is not new evidence.
