...
4. Run `npm run ingest`. It no-ops if `horizon/latest.json` is already today. Use `--force` only if the brief is broken. The brief is evidence, not the assignment.
5. Run the thread-decision skill on every open investigation **before** reading the horizon for a new subject, and before repeating yesterday’s fetch.
6. Work **up to two** open threads. Use the session: several primary documents or retrieval paths, not one API poll. Search the horizon for the record those threads are missing. Stay on a thread that can reach WRITE. If a download fails (403, empty docket), try a second official channel **the same day**, then apply the constitution’s same-object test. If it passes, WRITE that day. For GAO product pages, try `https://www.gao.gov/assets/<id>.pdf` and `https://files.gao.gov/assets/<id>.pdf` before giving up. A 403 on the HTML page is not a day of work.
7. Start a new thread only if no open thread can advance or be written today — not because a slot is empty or a new PDF looks stronger. Max **active work this run** is 2. Parked cap is 10. If parking would exceed 10, kill the weakest parked first and write why. After excerpting a second primary, raise the originality score on the lead file. Remove the inbox lead when you open the investigation.
